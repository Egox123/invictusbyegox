...
void C_LogManager::Instance()
{
	for (int32_t i = 0; i < m_Logs.size(); i++)
	{
		...
		uint8_t aColor[4] = {
			(unsigned char)g_Settings->m_colMenuTheme->r(),
			(unsigned char)g_Settings->m_colMenuTheme->g(),
			(unsigned char)g_Settings->m_colMenuTheme->b(),
			(unsigned char)255
		};
		...
		if (!LogData->m_bPrinted)
		{
			...
			g_Globals.m_Interfaces.m_CVar->ConsoleColorPrintf(aColor, _S("invictus"));
			g_Globals.m_Interfaces.m_CVar->ConsoleColorPrintf(aColor, _S(" ] "));
		...
		if (LogData->m_LogString.c_str()[0] == 'H')
		{
			if (LogData->m_LogString.c_str()[1] == 'I')
				g_Render->RenderRectFilled(0, flSpacing, flTextLength, flSpacing + 22.0f, Color(3, 252, 115, (int)(LogData->m_flBackAlpha)));
			else 
				g_Render->RenderRectFilled(0, flSpacing, flTextLength, flSpacing + 22.0f, Color(0, 0, 0, (int)(LogData->m_flBackAlpha)));
		}
		else if (LogData->m_LogString.c_str()[0] == 'M')
			g_Render->RenderRectFilled(0, flSpacing, flTextLength, flSpacing + 22.0f, Color(252, 3, 74, (int)(LogData->m_flBackAlpha)));
		else 
			g_Render->RenderRectFilled(0, flSpacing, flTextLength, flSpacing + 22.0f, Color(0, 0, 0, (int)(LogData->m_flBackAlpha)));
		
		/*if (LogData->m_LogIcon == _S("d"))
			g_Render->RenderText(_S("[B]"), ImVec2(4, flSpacing + 2),
				Color(g_Settings->m_colMenuTheme->r(), g_Settings->m_colMenuTheme->g(), g_Settings->m_colMenuTheme->b(), (int)(LogData->m_flTextAlpha)),
				false, false, g_Globals.m_Fonts.m_LogFont);
		else
			g_Render->RenderText(_S("[i]"), ImVec2(4, flSpacing + 2),
				Color(g_Settings->m_colMenuTheme->r(), g_Settings->m_colMenuTheme->g(), g_Settings->m_colMenuTheme->b(), (int)(LogData->m_flTextAlpha)),
				false, false, g_Globals.m_Fonts.m_LogIcons);*/
