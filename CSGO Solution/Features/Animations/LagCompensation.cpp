...
void C_LagCompensation::Instance( ClientFrameStage_t Stage )
{
	...
	for ( int32_t iPlayerID = 1; iPlayerID <= g_Globals.m_Interfaces.m_GlobalVars->m_iMaxClients; iPlayerID++ )
	{
		...
		if ( !pPlayer || !pPlayer->IsPlayer( ) || !pPlayer->IsAlive( ) || pPlayer->m_iTeamNum( ) == g_Globals.m_LocalPlayer->m_iTeamNum( ) )
		{
			...
			g_Resolver->DormancyReset( iPlayerID );
		...
		{
			if ( LagRecord.m_AnimationLayers.at( ANIMATION_LAYER_ALIVELOOP ).m_flCycle == this->GetPreviousRecord( iPlayerID ).m_AnimationLayers.at( ANIMATION_LAYER_ALIVELOOP ).m_flCycle )
				continue;
...
{
	std::memcpy( LagRecord.m_AnimationLayers.data( ), pPlayer->m_AnimationLayers( ), sizeof( C_AnimationLayer ) * ANIMATION_LAYER_COUNT );
...
float flServerTime(C_UserCmd* cmd) noexcept
{
	static int tick;
	static C_UserCmd* lastCmd;

	if ( cmd ) {
		if ( g_Globals.m_LocalPlayer && (!lastCmd || lastCmd->m_bHasBeenPredicted) )
			tick = g_Globals.m_LocalPlayer->m_nTickBase();
		else
			tick++;
		lastCmd = cmd;
	}
	return tick * m_Globals()->m_flIntervalPerTick;
}
