...
// disable fakelag with active exploits.
// #define DISABLE_FL_ON_EXPLOIT

#define DEV_BUILD

#define m_Globals() g_Globals.m_Interfaces.m_GlobalVars
...
public:
	...
	std::array < C_AnimationLayer, ANIMATION_LAYER_COUNT > m_AnimationLayers = { };
...
typedef Vector* (__thiscall* BuildTransformations_t)(void*, studiohdr_t*, Vector*, Quaternion*, matrix3x4_t*, int, byte*);
typedef void(__thiscall* IsDepthOfFieldEnabled_t)(LPVOID);
...
typedef void( __thiscall* ProcessPacket_t )( C_NetChannel*, LPVOID, bool );
...
typedef QAngle* (__thiscall* GetEyeAngles_t)(LPVOID);
typedef void (__thiscall* GetExposureRange_t)( LPVOID, float_t*, float_t* );
typedef int (__thiscall* SendDataGram_t)( C_NetChannel*, bf_write* );
...
public:
	...
	struct
	{
		...
		bool m_bHasMaximumAccuracy = false;
		/*bool m_bForceAutoPeek = true;*/
		bool m_bDoingSecondShot = false;
		bool m_bSlowWalkState = false;
	} m_AccuracyData;
	...
	bool m_bRenderInputTxtFrame = true;
	...
	struct
	{
		...
		int32_t m_iFlags[2];
		int32_t m_iMoveType[2];

		Vector m_vecNetworkedOrigin = Vector( 0, 0, 0 );
		Vector m_vecShootPosition = Vector( 0, 0, 0 );

		C_BasePlayer* m_LastBuiltTransformationEntity = nullptr;
		QAngle m_LastBuiltTransformationAngles = QAngle( 0, 0, 0 );
		bool m_bDidShotAtChokeCycle = false;
		QAngle m_angShotChokedAngle = QAngle( 0, 0, 0 );

		float_t m_flLastVelModifier = 0.0f;
	...
	struct
	{
		...
		std::array < bool, 65 > m_DidHitPrevious = { };
	} m_ResolverData;
	...
	struct
	{
		...
		uint8_t* m_ThirdPersonLean = nullptr;
		...
		uint8_t* m_PhysicsUnknwnFn = nullptr;
		uint8_t* m_LineThroughSmoke = nullptr;
	} m_AddressList;
	...
	struct
	{
		...
		ConVar* m_ClBobamtLat;
		ConVar* m_ClBombLowerAmt;
		ConVar* m_ClBobamtVert;
	...
	struct
	{
		...
		ImFont* m_WeaponMass = nullptr;
		ImFont* m_BombIcon = nullptr;
		ImFont* m_TitleFont = nullptr;
		ImFont* m_ESP = nullptr;
		ImFont* m_Flags = nullptr;
		ImFont* m_Damage = nullptr;
		ImFont* m_Warning = nullptr;
	} m_Fonts;
	...
	struct
	{
		struct
		{
			...
			GetExposureRange_t m_GetExposureRange = NULL;
			...
			C_RecvHook* m_bUseCustomBloomScale;
			C_RecvHook* m_flCustomBloomScale;
			C_RecvHook* m_bUseCustomAutoExposureMin;
			C_RecvHook* m_flCustomAutoExposureMin;
			C_RecvHook* m_bUseCustomAutoExposureMax;
			C_RecvHook* m_flCustomAutoExposureMax;
			C_RecvHook* m_SequenceProxy;
			C_RecvHook* m_SimulationTime;
			...
			BuildTransformations_t m_BuildTransformations = NULL;
			IsDepthOfFieldEnabled_t m_IsDepthOfFieldEnabled = NULL;
			GetEyeAngles_t m_GetEyeAngles = NULL;
			SendDataGram_t m_SendDataGram = NULL;
		} m_Originals;

		bool m_bDidHookDataGram = false;
	} m_Hooks;
	...
	std::deque< CorrectionData > m_CorrectData = { };
