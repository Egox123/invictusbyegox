...
void C_SetUp::Instance( HINSTANCE hInstance )
{
	...
	// hooked.
	MessageBoxA(NULL, _S("Cheat has successfully injected."), _S("invictus by egox"), 64);

	// get out of thread
	return ExitThread( EXIT_SUCCESS );
}

#define CREATE_HOOK( Address, Function, Original ) if ( MH_CreateHook( ( LPVOID )( Address ), ( LPVOID )( Function ), reinterpret_cast< void** >( &Original ) ) ) MessageBoxA( NULL, _S( "liquid smoke.dll" ), _S( #Function ), 64 ); 
...
void C_SetUp::SetupHooks( )
{
	...
	LPVOID pCreateMove                  = ( LPVOID )( ( *( uintptr_t** )( g_Globals.m_Interfaces.m_CHLClient) )[ 22 ] );
	...
	LPVOID pViewModelCallback			= g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "55 8B EC 83 E4 F8 A1 ?? ?? ?? ?? 83 EC 08 B9 ?? ?? ?? ?? FF 50 ?? 6A 00 85 C0 74 23 83 EC 08 C7 44 24 04 00 00 88" ) );
	LPVOID pGetViewmodelFOV				= g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "55 8B EC 8B 4D 04 83 EC 08 57") );
	LPVOID pSendNetMessage				= g_Tools->FindPattern( g_Globals.m_ModuleList.m_EngineDll, _S( "55 8B EC 83 EC 08 56 8B F1 8B 4D 04 E8" ) );
	...
	LPVOID pIsUsingDebugStaticProps		= g_Tools->FindPattern( g_Globals.m_ModuleList.m_EngineDll, _S( "E8 ? ? ? ? 84 C0 8B 45 08" ) );
	...
	LPVOID pPlayerHook					= g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 89 7C" ) ) + 0x47;
	LPVOID pGetEyeAngles				= (LPVOID)((*(uintptr_t**)(pPlayerHook))[170]);
	LPVOID pBuildTransformations		= (LPVOID)((*(uintptr_t**)(pPlayerHook))[190]);
	LPVOID pSimulationTime				= g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll, _S("55 8B EC 8B 45 08 8B 4D 0C 53 83 C1 08 56 8B 70 08 57 8B 01 FF 50 28 8B ? ? ? ? ? 99 8B"));
	LPVOID pDepthOfFiledEnabled			= g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll, _S("8B ? ? ? ? ? 56 8B 01 FF 50 34 8B F0 85 F6 75"));
	LPVOID pGetExposureRange			= g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll, _S("55 8B EC 51 80 3D ? ? ? ? ? 0F 57"));
	LPVOID pSetViewModel				= g_Tools->FindPattern(g_Globals.m_ModuleList.m_ClientDll, _S("55 8B EC 8B 45 08 F3 0F 7E 45"));
	LPVOID pProcessPacket				= g_Tools->FindPattern(g_Globals.m_ModuleList.m_EngineDll, _S("55 8B EC 83 E4 C0 81 EC ? ? ? ? 53 56 57 8B 7D 08 8B D9"));
	LPVOID pDataGram					= g_Tools->FindPattern(g_Globals.m_ModuleList.m_EngineDll, _S("55 8B EC 83 E4 F0 B8 38"));
	...
	CREATE_HOOK( pProcessPacket					,		C_Hooks::hkProcessPacket				,		g_Globals.m_Hooks.m_Originals.m_ProcessPacket				);
	...
	CREATE_HOOK( pBuildTransformations			,		C_Hooks::hkBuildTransformations			,		g_Globals.m_Hooks.m_Originals.m_BuildTransformations		);
	CREATE_HOOK( pDepthOfFiledEnabled			,		C_Hooks::hkIsDepthOfFieldEnabledHook	,		g_Globals.m_Hooks.m_Originals.m_IsDepthOfFieldEnabled		);
	CREATE_HOOK( pDataGram						,		C_Hooks::hkSendDataGram					,		g_Globals.m_Hooks.m_Originals.m_SendDataGram				);
	...
	//CREATE_HOOK( pGetExposureRange				,		C_Hooks::hkGetExposureRange				,		g_Globals.m_Hooks.m_Originals.m_GetExposureRange			);
	//CREATE_HOOK( pGetEyeAngles					,		C_Hooks::hkGetEyeAngles					,		g_Globals.m_Hooks.m_Originals.m_GetEyeAngles				);
	...
	g_Globals.m_Hooks.m_Originals.m_SequenceProxy	= new C_RecvHook( g_NetvarManager->GetNetProp( _S( "DT_BaseViewModel"	), _S( "m_nSequence"		) ), C_Hooks::hkSequenceProxy );
}
...
void C_SetUp::SetupAddressList( )
{
	...
	g_Globals.m_AddressList.m_ThirdPersonLean						=	g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "8B 55 0C 8B C8 E8 ? ? ? ? 83 C4 08 5E 8B E5" ));
	g_Globals.m_AddressList.m_PhysicsRunThink						=	g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "55 8B EC 83 EC 10 53 56 57 8B F9 8B ? ? ? ? ? C1" )); 
	g_Globals.m_AddressList.m_PhysicsUnknwnFn						=	g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "55 8B EC 56 57 8B F9 8B B7 ? ? ? ? 8B" ));
	g_Globals.m_AddressList.m_LineThroughSmoke						=	g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "55 8B EC 83 EC 08 8B 15 ? ? ? ? 0F 57 C0" ));
}
...
void C_SetUp::SetupConvarList( )
{
	...
	g_Globals.m_ConVars.m_ClBobamtLat					= g_Globals.m_Interfaces.m_CVar->FindVar( _S( "cl_bobamt_lat" ) );
	g_Globals.m_ConVars.m_ClBombLowerAmt				= g_Globals.m_Interfaces.m_CVar->FindVar( _S( "cl_bob_lower_amt" ) );
	g_Globals.m_ConVars.m_ClBobamtVert					= g_Globals.m_Interfaces.m_CVar->FindVar( _S( "cl_bobamt_vert" ) );
	...
	g_Globals.m_ConVars.m_OptimizePred					= g_Globals.m_Interfaces.m_CVar->FindVar( _S( "cl_pred_optimize" ) );
	g_Globals.m_ConVars.m_Ambient						= g_Globals.m_Interfaces.m_CVar->FindVar( _S( "r_modelAmbientMin" ) );
...
void C_SetUp::SetupInterfaceList( )
{
	...
	g_Globals.m_Interfaces.m_Effects			= reinterpret_cast<IEffects*>(g_Tools->GetInterface(g_Globals.m_ModuleList.m_ClientDll, _S("IEffects001")));
	...
	g_Globals.m_Interfaces.m_GameRules			= *( C_GameRules*** )( g_Tools->FindPattern( g_Globals.m_ModuleList.m_ClientDll, _S( "E8 ? ? ? ? A1 ? ? ? ? 85 C0 0F 84 83 ? ? ?" ) ) + 0x6 );
