...
LRESULT WINAPI C_Hooks::hkWndProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	if ( ImGui_ImplWin32_WndProcHandler( hWnd, msg, wParam, lParam ) && g_Menu->IsMenuOpened( ) && !bPressedMovementKeys && !g_Globals.m_KeyData.m_bFocusedInput )
		return true;

	if ( g_Menu->IsMenuOpened( ) && bPressedMovementKeys && !g_Globals.m_KeyData.m_bFocusedInput )
		return false;
