...
void __fastcall C_Hooks::hkGetExposureRange( LPVOID pEcx, uint32_t, float_t* pflExposureMin, float_t* pflExposureMax )
{
	if ( g_Settings->m_bAmbientModulation )
	{
		*pflExposureMin = g_Settings->m_flExposure;
		*pflExposureMax = g_Settings->m_flExposure;
	}
	else
	{
		*pflExposureMin = 1.f;
	...
	//g_Globals.m_Hooks.m_Originals.m_GetExposureRange(pflExposureMin, pflExposureMax);
}