...
public:
	...
	static int __fastcall hkSendDataGram(C_NetChannel* pNetwork, LPVOID pEcx, bf_write* pDataGram);
	static void __fastcall hkProcessPacket(C_NetChannel* pNetwork, LPVOID pEcx, LPVOID pPacket, bool pHeader);
	...
	static void __fastcall hkBuildTransformations(void* ecx, void* edx, studiohdr_t* hdr, Vector* pos, Quaternion* q, matrix3x4_t* cameraTransform, int boneMask, byte* boneComputed);

	static void hkFlashDuration( CRecvProxyData* Data, LPVOID pStruct, LPVOID pOut );
	...
	static void __fastcall hkGetExposureRange( LPVOID pEcx, uint32_t, float_t* pflExposureMin, float_t* pflExposureMax );
	...
	static bool __stdcall hkIsDepthOfFieldEnabledHook() noexcept;
	static int __fastcall hkSetViewModel(uintptr_t ecx, uintptr_t edx, int unk, float x, float y, float z);
	...
	static void hkSequenceProxy( CRecvProxyData* Proxy, LPVOID pStruct, LPVOID pOut );
