...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (held)
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(0, 0, 0, (int)(g.Style.Alpha * 255)), 4.0f),
        window->DrawList->AddRect(bb.Min, bb.Max, ImColor(g_Settings->m_colMenuTheme->r(), g_Settings->m_colMenuTheme->g(), g_Settings->m_colMenuTheme->b(), (int)(g.Style.Alpha * 255)), 4.0f);
    else if (hovered)
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(g_Settings->m_colMenuTheme->r(), g_Settings->m_colMenuTheme->g(), g_Settings->m_colMenuTheme->b(), (int)(g.Style.Alpha * 255)), 4.0f),
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(0, 0, 0, (int)(g.Style.Alpha * 50)), 4.0f);
    else 
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(g_Settings->m_colMenuTheme->r(), g_Settings->m_colMenuTheme->g(), g_Settings->m_colMenuTheme->b(), (int)(g.Style.Alpha * 255)), 4.0f);
...
#include "../Features/KeyBind Manager/KeyBind Manager.hpp"

void ImGui::KeyBindPopUp(void* v, const char* label, int8_t data_constant, const std::vector < const char* > * arr_data)
{
    C_Binding aBind;

    // from origin to dest.., origin is LPVOID to a direct value, not byte array.
    switch ( data_constant )
    {
    case 0:// bool
        // use bit manipulation to store data.
        bool b;
        ImGui::Checkbox(label, &b);
            
        g_KeyBindManager->AddBind(&aBind, v, &b, data_constant, label);
        break;
    case 1: // int
        int32_t i;
        ImGui::SliderInt(label, &i, 0, 120);
        g_KeyBindManager->AddBind(&aBind, v, &i, data_constant, label);
        break;
    case 2: // float
        float_t f;
        ImGui::SliderFloat(label, &f, 0, 120);
        g_KeyBindManager->AddBind(&aBind, v, &i, data_constant, label);
        break;
    default:// array<bool, x>
        // use bit manipulation to store data.
        int8_t nSizeOfArr = data_constant - KEYBIND_TYPE::KEYBIND_ARRAY;
    ...
    auto _var = false; // new value!

    g_KeyBindManager->AddBind(&aBind, v, &_var, data_constant, label);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddLine(check_bb.Min + ImVec2(0, 5), check_bb.Min + ImVec2(28, 5), ImColor(10, 10, 10, (int)(g.Style.Alpha * 140.f)), 2);
    ...
    if (*v)
    {
        ...
        window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(it_pos_active->second, 6), it_size_active->second, ImColor(g_Settings->m_colCheckbox->r(), g_Settings->m_colCheckbox->g(), g_Settings->m_colCheckbox->b(), (int)(g.Style.Alpha * 255)), 16);
    }
    else
    {
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(pos.x - 3, total_bb.Min.y - 5), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(w + 5, label_size.y + 4));
    ...
    window->DrawList->AddRectFilled(cb_bb.Min, cb_bb.Max, ImColor(20, 20, 20, (int)(g.Style.Alpha * 140.f)), 4.0f, popup_open ? 3 : 0);
    ...
    RenderText(cb_bb.Min + ImVec2(5, 1), preview_value);
    window->DrawList->PopClipRect();

    if (!popup_open)
        RenderArrow(window->DrawList, cb_bb.Max - ImVec2(16, 14), ImColor(g_Settings->m_colCombo->r(),g_Settings->m_colCombo->g(),g_Settings->m_colCombo->b(), 255), ImGuiDir_Down, 0.5f);
    else
        RenderArrow(window->DrawList, cb_bb.Max - ImVec2(16, 14), ImColor(g_Settings->m_colCombo->r(),g_Settings->m_colCombo->g(),g_Settings->m_colCombo->b(), 255), ImGuiDir_Up, 0.5f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled(cb_bb.Min, cb_bb.Max, ImColor(15, 15, 15, (int)(g.Style.Alpha * 255)), 4.0f, popup_open ? 3 : 15);
...
bool ImGui::Keybind(const char* str_id, int* current_key, int* key_style) {
    ...
    const ImVec2 label_size = CalcTextSize("None");
    PopFont();
    const ImRect frame_bb(window->DC.CursorPos - ImVec2(5, 0) + ImVec2(0, 1), window->DC.CursorPos + ImVec2(label_size.x + 5, label_size.y + 4) - ImVec2(5, 0) + ImVec2(0, 3));
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min), ImVec2(frame_bb.Max), ImColor(20, 20, 20, (int)(g.Style.Alpha * 140.f)), 1.0f);
    //window->DrawList->AddRect(ImVec2(frame_bb.Min), ImVec2(frame_bb.Max), ImColor(38, 38, 42, (int)(g.Style.Alpha * 255)), 2.0f);

    ImVec2 newLabelSize = ImGui::CalcTextSize(label_display);
    newLabelSize.x = (frame_bb.Max.x - frame_bb.Min.x) - newLabelSize.x;
    newLabelSize.x /= 2;

    window->DrawList->AddText(frame_bb.Min + ImVec2(newLabelSize.x, 2), g.ActiveId == id ? ImColor(255, 255, 255) : ImColor(g_Settings->m_colKeybind->r() / 255.f, g_Settings->m_colKeybind->g() / 255.f, g_Settings->m_colKeybind->b() / 255.f, g.Style.Alpha), buf_display);
    PopFont();
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
   ...
   if (it_size == size.end())
    ...
    window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.Min.y), ImVec2(slider_bb.Max.x, slider_bb.Max.y), ImColor(20, 20, 20, (int)(g.Style.Alpha * 140.f)));
    //window->DrawList->AddRect(slider_bb.Min, slider_bb.Max, GetColorU32(ImVec4(38 / 255.0f, 38 / 255.0f, 42 / 255.0f, g.Style.Alpha)), 4.0f);
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y), ImColor(g_Settings->m_colSlider->r(), g_Settings->m_colSlider->g(), g_Settings->m_colSlider->b(), (int)(g.Style.Alpha * 255)), 2);
    ...
    //if (strcmp(label, _S("Minimum damage")) || strcmp(label, _S("Override damage")))
    //{
    //    // fucked up as hell.
    //    memset(value_buf, 0, sizeof(value_buf));

    //    char dmg_buff[64];
    //    snprintf(dmg_buff, sizeof(dmg_buff), "%d", it_active_value->first - 100 );

    //    strncat_s(value_buf, "HP + ", 4);
    //    strncat_s(value_buf, dmg_buff, strlen(dmg_buff));
    //}

    const ImVec2 vStartPos  = ImVec2( slider_bb.Max.x + 4, slider_bb.Max.y - CalcTextSize(value_buf).y / 2 - 2 ); 
    const ImVec2 vEndPos    = ImVec2( slider_bb.Max.x + 36, slider_bb.Min.y + CalcTextSize(value_buf).y / 2 + 2 );
    
    ImVec2 vTextPos   = ImVec2( slider_bb.Max.x + 20 - ( CalcTextSize(value_buf).x / 2 ), vStartPos.y - 2);
    
    /*
        g_Globals.m_bRenderInputTxtFrame = false;

    ImGui::SetCursorPos(vStartPos);

    if ( data_type == ImGuiDataType_S32 )
        ImGui::InputInt((label), (int*)p_data, 0),
        std::clamp(*(int*)p_data, *(int*)p_min, *(int*)p_max);
    else
        ImGui::InputFloat((label), (float*)p_data, 0),
        std::clamp(*(float*)p_data, *(float*)p_min, *(float*)p_max);

    g_Globals.m_bRenderInputTxtFrame = true;
    */

    //window->DrawList->AddRectFilled(vStartPos, vEndPos, ImColor(20, 20, 20, (int)(g.Style.Alpha * 140.f)), 0.f);
    //window->DrawList->AddRect(vStartPos, vEndPos, ImColor(g_Settings->m_colSlider->r() / 255.f, g_Settings->m_colSlider->g() / 255.f, g_Settings->m_colSlider->b() / 255.f, g.Style.Alpha), 4.f);
    //window->DrawList->AddText(vTextPos, ImColor(1.f, 1.f, 1.f, g.Style.Alpha), value_buf);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* data_ptr, const void* step, const void* step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (step != NULL)
    {
        ...
        PushID(label);
        SetNextItemWidth(ImMax(1.0f, !g_Globals.m_bRenderInputTxtFrame ? 30 : 200 - (button_size) * 2));
        ...
        const char* label_end = FindRenderedTextEnd(label);
        if (label != label_end && g_Globals.m_bRenderInputTxtFrame )
        {
            SameLine(0, style.ItemInnerSpacing.x);
            TextEx(label, label_end);
        }
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = !g_Globals.m_bRenderInputTxtFrame ? ImVec2(65, label_size.y + style.FramePadding.y * 2.0f) : CalcItemSize(size_arg, 235, (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y*2.0f); // Arbitrary default of 8 lines high for multi-line
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));

    g_Globals.m_KeyData.m_bFocusedInput = true;
    ...
    {
        ClearActiveID();
        g_Globals.m_KeyData.m_bFocusedInput = false;
    }
    ...
    if (!is_multiline )
    {
        if ( g_Globals.m_bRenderInputTxtFrame ) {
            window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max - ImVec2(0, 2), ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 4.0f);
            window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(g_Settings->m_colMenuTheme->r() / 255.0f, g_Settings->m_colMenuTheme->g() / 255.0f, g_Settings->m_colMenuTheme->b() / 255.0f, g.Style.Alpha)), 4.0f);
        }
        else {
            window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(20, 20, 20, (int)(g.Style.Alpha * 140.f)), 0.0f);
        }
    }
    ...
    if (label_size.x > 0 && g_Globals.m_bRenderInputTxtFrame )
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, int allign_mode)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    
    //(allign_mode == 2 ? ImVec2(35, 5) : allign_mode == 1 ? ImVec2(0, 2)
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size, int allign_mode)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect bb(window->DC.CursorPos + cRot, window->DC.CursorPos + cRot + size);
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(22, 22, 26, (int)(g.Style.Alpha * 255)), 2.0f);
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImVec4(38 / 255.0f, 38 / 255.0f, 42 / 255.0f, g.Style.Alpha)), 2.0f);
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f)
    {
        float mid_x = (float)(int)((bb_inner.Min.x + bb_inner.Max.x) * 0.5f + 0.5f);
        RenderColorRectWithAlphaCheckerboard(ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawCornerFlags_TopRight | ImDrawCornerFlags_BotRight);
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_without_alpha), 2.0f, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_BotLeft);
    ...
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 2.0f, 0);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 2.0f, ImDrawCornerFlags_All);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 size(size_arg.x != 0.0f ? size_arg.x + 10 : label_size.x + 10, size_arg.y != 0.0f ? size_arg.y : label_size.y);
    ImVec2 pos = window->DC.CursorPos;
    ...
    float w_draw = ImMax(label_size.x + 10, window->Pos.x + max_x - window_padding.x - pos.x + 10);
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(40, 40, 40, 255));

    if ( hovered || selected )
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(0, 0, 0, 160));

    const ImVec4 hover_act = ImVec4(g_Settings->m_colCombo->r() / 255.f, g_Settings->m_colCombo->g() / 255.f, g_Settings->m_colCombo->b() / 255.f, 1.f);
    const ImVec4 hover_dis = ImVec4(g_Settings->m_colCombo->r() / 255.f, g_Settings->m_colCombo->g() / 255.f, g_Settings->m_colCombo->b() / 255.f, 0.8f);
    float deltatime = 1.5f * ImGui::GetIO().DeltaTime;
...
bool ImGui::CustomSelectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const float max_x = GetContentRegionMaxAbs().x;
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, bool custom_selectable)
{
    ...
    while (clipper.Step())
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++)
        {
            ...
            PushID(i);
            if (Selectable(item_text, item_selected, 0, ImVec2(280.f, 0.f)))
            {
                *current_item = i;
                value_changed = true;
            }
