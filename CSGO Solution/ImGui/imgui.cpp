...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
   ...
   /* if (!g_Menu->ShouldChildFrameOutputText())
    {
        parent_window->DrawList->AddText({ ImGui::GetWindowPos().x + 13,ImGui::GetWindowPos().y - 20 }, ImColor(g_Settings->m_colTabText->r() / 255.f, g_Settings->m_colTabText->g() / 255.f, g_Settings->m_colTabText->b() / 255.f, g.Style.Alpha), name);
    }*/
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    const float window_rounding = 0;//window->WindowRounding;
    ...
    else
    {
        // Window background
        {
            ...
            {
                PushFont(g_Globals.m_Fonts.m_TitleFont);
                
                window->DrawList->AddRectFilled({ window->Pos.x, window->Pos.y }, (window->Pos + window->Size), ImColor(30, 30, 30, alpha));

                /*window->DrawList->AddRectFilledMultiColor({ window->Pos.x, window->Pos.y }, {window->Pos + ImVec2(600, 50)}, 
                    ImColor(g_Settings->m_colMenuTheme->r()/ 255.f, g_Settings->m_colMenuTheme->g()/ 255.f, g_Settings->m_colMenuTheme->b() / 255.f, g_Settings->m_colMenuTheme->a() / 255.f),
                    ImColor(g_Settings->m_colMenuTheme->r()/ 255.f, g_Settings->m_colMenuTheme->g()/ 255.f, g_Settings->m_colMenuTheme->b() / 255.f, g_Settings->m_colMenuTheme->a() / 255.f),
                    ImColor(g_Settings->m_colMenuTheme->r()/ 255.f, g_Settings->m_colMenuTheme->g()/ 255.f, g_Settings->m_colMenuTheme->b() / 255.f, 0.f),
                    ImColor(g_Settings->m_colMenuTheme->r()/ 255.f, g_Settings->m_colMenuTheme->g()/ 255.f, g_Settings->m_colMenuTheme->b() / 255.f, 0.f)
                );*/

                window->DrawList->AddRectFilled({ window->Pos.x ,window->Pos.y }, (window->Pos + ImVec2( window->Size.x, 80 )), ImColor(20, 20, 20, alpha));        
                window->DrawList->AddRectFilled({ window->Pos.x ,window->Pos.y + 650 }, (window->Pos + window->Size), ImColor(20, 20, 20, alpha));

                window->DrawList->AddRectFilledMultiColor({ window->Pos.x ,window->Pos.y + 650 }, (window->Pos + ImVec2(window->Size.x, 620)),                 
                    ImColor(0, 0, 0, (int)(g.Style.Alpha * 110)), ImColor(0, 0, 0, (int)(g.Style.Alpha * 110)),
                    ImColor(0, 0, 0, (int)(g.Style.Alpha * 0)), ImColor(0, 0, 0, (int)(g.Style.Alpha * 0))); // shader.
                window->DrawList->AddRectFilledMultiColor({ window->Pos.x ,window->Pos.y + 80}, (window->Pos + ImVec2(window->Size.x, 110)),                 
                    ImColor(0, 0, 0, (int)(g.Style.Alpha * 110)), ImColor(0, 0, 0, (int)(g.Style.Alpha * 110)),
                    ImColor(0, 0, 0, (int)(g.Style.Alpha * 0)), ImColor(0, 0, 0, (int)(g.Style.Alpha * 0))); // shader.

                //window->DrawList->AddRectFilled({ window->Pos.x ,window->Pos.y + 649 }, (window->Pos + ImVec2(window->Size.x, 650)), ImColor(220, 220, 220, alpha)); // line split.

                const ImVec2 LogoSize = ImGui::CalcTextSize(_S("invictus"));    
                const ImVec2 LogoPosition = ImVec2( window->Pos.x + 300 - ( LogoSize.x * 0.5f), window->Pos.y + 5 );
                window->DrawList->AddText(LogoPosition, ImColor(g_Settings->m_colMenuTheme->r()/ 255.f, g_Settings->m_colMenuTheme->g()/ 255.f, g_Settings->m_colMenuTheme->b() / 255.f, 1.f), _S("invictus"));
                ImGui::PopFont();

                ImGui::PushFont(ImGui::GetIO().Fonts->Fonts[0]);
                const ImVec2 LogoSize2 = ImGui::CalcTextSize(_S("FREEWARE BY EGOX"));

                window->DrawList->AddRectFilled({ window->Pos.x ,window->Pos.y + 79}, 
                    (window->Pos + ImVec2((300 - ( LogoSize2.x  / 2 ) - 5), 81)), 
                    ImColor(g_Settings->m_colMenuTheme->r(), g_Settings->m_colMenuTheme->g(), g_Settings->m_colMenuTheme->b(), g_Settings->m_colMenuTheme->a()));
                window->DrawList->AddRectFilled({ window->Pos.x + 300 + 5 + (LogoSize2.x / 2),window->Pos.y + 79}, 
                    (window->Pos + ImVec2(window->Size.x, 81)), 
                    ImColor(g_Settings->m_colMenuTheme->r(), g_Settings->m_colMenuTheme->g(), g_Settings->m_colMenuTheme->b(), g_Settings->m_colMenuTheme->a()));

                const ImVec2 LogoPosition2 = ImVec2( window->Pos.x + 300 - ((LogoSize2.x) * 0.5f), window->Pos.y + LogoSize.y + 5);
                window->DrawList->AddText(LogoPosition2, ImColor(1.0f,1.0f,1.0f,1.0f), _S("FREEWARE BY EGOX"));
            ...
            else 
            {
                ...
                {
                    draw->AddRectFilled(window->Pos, window->Pos + window->Size, ImColor(30, 30, 30, 140), 0.0F, ImDrawCornerFlags_Bot);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->WindowRounding = (flags & ImGuiWindowFlags_ChildWindow) ? style.ChildRounding : ((flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Modal)) ? style.PopupRounding : 0.f;
...
void ImGui::SameLine(float offset_from_start_x, float spacing_w)
{
    ...
    else
    {
        ...
        window->DC.CursorPos.y = !g_Globals.m_bRenderInputTxtFrame ? window->DC.CursorPosPrevLine.y + 7 : window->DC.CursorPosPrevLine.y;
    }
